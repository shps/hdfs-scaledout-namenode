...
node.set_unless['mysql']['server_debian_password'] = "kthfs"
node.set_unless['mysql']['server_root_password']   = "kthfs"
node.set_unless['mysql']['server_repl_password']   = "kthfs"
node.set_unless['mysql']['kthfs_password']   = "kthfs"
...
if platform?(%w{mac_os_x})
  ...
  kthfs_path = node['mysql']['kthfs_path']
  ...
  begin
    ...
    Chef::Log.info("Could not find previously defined kthfs.sql resource")
    t = template kthfs_path do
      source "kthfs.sql.erb"
      owner "root" unless platform? 'windows'
      group node['mysql']['root_group'] unless platform? 'windows'
      mode "0600"
      action :create
  ...
  if platform? 'windows'
    ...
    execute "mysql-install-kthfs" do
      command "\"#{node['mysql']['mysql_bin']}\" -u root #{node['mysql']['server_root_password'].empty? ? '' : '-p' }\"#{node['mysql']['server_root_password']}\" kthfs < \"#{kthfs_path}\""
      action :nothing
      subscribes :run, resources("template[#{kthfs_path}]"), :immediately
    end
