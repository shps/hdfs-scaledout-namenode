 ...
 *
 * @author Jude
 * This tests basic failover b/w two namenodes in the system
 */
public class TestHABasicFailover  extends junit.framework.TestCase
{
  ...
  // ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  /**
   * Testing basic failover.
   * After starting namenodes NN1, NN2, the namenode that first initializes itself would be elected the leader.
   * We allow NN1 to be the leader. 
   * We kill NN1. Failover will start and NN2 will detect failure of NN1 and hence would elect itself as the leader
   * Also perform fail-back to NN1 by killing NN2
   */
  @Test
  public void testFailover() {
    ...
    try {
      ...
      waitLeaderElection(cluster.getDataNodes(), cluster.getNameNode(NN1), timeout);
  ...
  public static void waitLeaderElection(List<DataNode> datanodes, NameNode nn, long timeout) throws TimeoutException{
    ...
    while(true) {
      ...
      // check for time out
      if(System.currentTimeMillis() - initTime >= timeout) {
        throw new TimeoutException("Datanodes wasn't able to detect newly elected leader");
      }
    }
  }
}
