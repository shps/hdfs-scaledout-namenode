...
public class TestDFSStorageStateRecovery extends TestCase {
     ...
     * exists.
     *
     * The column meanings are: 0) current directory exists 1) previous
     * directory exists 2) previous.tmp directory exists 3) removed.tmp
     * directory exists 4) node should recover and startup 5) current directory
     * should exist after recovery but before startup 6) previous directory
     ...
     * {@link DFSConfigKeys#DFS_NAMENODE_NAME_DIR_KEY}. For each element in {@link DFSConfigKeys#DFS_NAMENODE_NAME_DIR_KEY},
     * the subdirectories represented by the first four elements of the
     * <code>state</code> array will be created and populated.
     *
     * See {@link UpgradeUtilities#createNameNodeStorageDirs()}
     *
     * @param state a row from the testCases table which indicates which
     ...
     * represented by the first four elements of the
     * <code>state</code> array will be created and populated. See {@link UpgradeUtilities#createDataNodeStorageDirs()}
     *
     * @param state a row from the testCases table which indicates which
     * directories to setup for the node
     ...
     * {@link DFSConfigKeys#DFS_DATANODE_DATA_DIR_KEY}. For each element in {@link DFSConfigKeys#DFS_DATANODE_DATA_DIR_KEY},
     * the subdirectories represented by the first four elements of the
     * <code>state</code> array will be created and populated. See {@link UpgradeUtilities#createBlockPoolStorageDirs()}
     *
    ...
    /**
     * For NameNode, verify that the current and/or previous exist as indicated
     * by the method parameters. If previous exists, verify that it hasn't been
     * modified by comparing the checksum of all it's containing files with
     * their original checksum. It is assumed that the server has recovered.
     */
    ...
    /**
     * For datanode, verify that the current and/or previous exist as indicated
     * by the method parameters. If previous exists, verify that it hasn't been
     * modified by comparing the checksum of all it's containing files with
     * their original checksum. It is assumed that the server has recovered.
     */
    ...
    /**
     * For block pool, verify that the current and/or previous exist as
     * indicated by the method parameters. If previous exists, verify that it
     * hasn't been modified by comparing the checksum of all it's containing
     * files with their original checksum. It is assumed that the server has
     * recovered.
     *
