...
public class TestBlockReport {
    ...
    private static final int REP_INTERVAL = 10;
    ...
    private void startUpCluster(boolean format) throws IOException {
        // set replication interval a bit longer for test03
        conf.setInt(DFSConfigKeys.DFS_NAMENODE_REPLICATION_INTERVAL_KEY, REP_INTERVAL);

        REPL_FACTOR = 1; //Reset if case a test has modified the value
        cluster = new MiniDFSCluster.Builder(conf).format(format).numNameNodes(2).numDataNodes(REPL_FACTOR).build();
        cluster.waitActive();
     ...
     * are messed up and BlockReport is forced. The modification of blocks'
     * length has to be ignored
     *
     ...
     * recalculate required DN's activities such as replications and so on. The
     * number of missing and under-replicated blocks should be the same in case
     * of a single-DN cluster.
     *
    ...
    /**
     * Test writes a file and closes it. Then test finds a block and changes its
     * GS to be < of original one. New empty block is added to the list of
     * blocks. Block report is forced and the check for # of corrupted blocks is
     * performed.
     *
     *
     *
    ...
    public void blockReport_03() throws IOException {
        ...
        // The block with modified GS won't be found. Has to be deleted // corrupt
        ...
        // The block report method would cause the (new) block to go the invalidate list
        // This test might fail as the replication monitor would delete the block from Invalidates
        // So we need to increase the replication monitor interval
    ...
    /**
     * This test isn't a representative case for BlockReport The empty method is
     * going to be left here to keep the naming of the test plan in synch with
    ...
    /**
     * Test creates a file and closes it. The second datanode is started in the
     * cluster. As soon as the replication process is completed test runs Block
     * report and checks that no underreplicated blocks are left
     *
     * @throws IOException in case of an error
     */
    @Test
    ...
    /**
     * Similar to BlockReport_03() but works with two DNs Test writes a file and
     * closes it. The second datanode is started in the cluster. As soon as the
     * replication process is completed test finds a block from the second DN
     * and sets its GS to be < of original one. Block report is forced and the
     * check for # of currupted blocks is performed. Another block is chosen and
     * its length is set to a lesser than original. A check for another
     * corrupted block is performed after yet another BlockReport
     *
     *
     *
     ...
     * restarts initiated single-node cluster. Then it writes a file >
     * block_size and closes it. The second datanode is started in the cluster.
     * As soon as the replication process is started and at least one TEMPORARY
     * replica is found test forces BlockReport process and checks if the
     * TEMPORARY replica isn't reported on it. Eventually, the configuration is
     * being restored into the original state.
     *
     * @throws IOException in case of an error
     */
    @Test
    ...
    private Block findBlock(Path path, long size) throws IOException {
        ...
        ret = cluster.getDataNodes().get(DN_N0).data.getStoredBlock(lb.getBlock()
