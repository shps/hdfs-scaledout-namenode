...
public abstract class INode implements Comparable<byte[]>, FSInodeInfo {
	 ...
	 * INode is deleted.
	 */
	abstract int collectSubtreeBlocksAndClear(List<Block> blocks, boolean isTransactional);
	...
	void setModificationTimeForceDB(long modtime, boolean isTransactional) {
          assert !isDirectory();
	 ...
	 */
	void setAccessTimeDB(long atime, boolean isTransactional) {
	  INodeHelper.updateAccessTime(this.id, atime, isTransactional);
	  accessTime = atime;
	}
