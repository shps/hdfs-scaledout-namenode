...
public class BlocksHelper2 {

	private static Log LOG = LogFactory.getLog(BlocksHelper2.class);
	...
	public static BlockInfo getBlockInfo(long blockId)  {
		...
		while (done == false && tries > 0) {
			try {
				...
				LOG.debug("inside getBlockInfo input:" + blockId);
				if(ret != null)
					LOG.debug("inside getBlockInfo output:" + ret.getBlockId());
				else
					LOG.debug("inside getBlockInfo output:" + null);
				return ret;
			}
			catch (ClusterJException e){
	...
	private static BlockInfo getBlockInfoInternal(long blockId, Session session, boolean single) {
		...
		if(bit == null)
		{
			LOG.debug("inside getBlockInfoInternal BlockInfoTable is null");
			return null;
		}
		else {
	...
	public static int getTripletsForBlockLength (BlockInfo blockinfo) {
		...
		while (done == false && tries > 0) {
			try {
				...
				LOG.debug("inside getTripletsForBlockLength length:"+ ret);
				done=true;
				return ret;
			}
			catch (ClusterJException e){
	 ...
	 */
	private BlockInfoTable selectBlockInfo(Session session, long blkid) {
	...
	private void insertBlockInfo(Session session, 
			long blockID, 
			int index, 
			long iNodeID, 
			long numBytes, 
	 ...
	 */
	private TripletsTable selectTriplet(Session session, long blkid, int index) {
		Object[] pKey = new Object[2];
		pKey[0] = blkid;
		pKey[1] = index;
	 ...
	 * @param index
	 */
	private void deleteTriplet(Session session, long blkid, int index) {
		Object[] pKey = new Object[2];
		pKey[0] = blkid;
		pKey[1] = index;
	...
	private void insertTriplet(Session session, TripletsTable tt) {
		session.makePersistent(tt);
	}
	
	@SuppressWarnings("unused")
	private void insertTriplet(Session session,
			long blkid,
			int index, 
			String datanodeName) {
		...
		insertTriplet(session, tt);
	}
	
	/** Fetch a row from the Triplets table using blockId //TODO: see if this works
	 * @param session
	 * @param blkid
	 * @return a row from the Triplets table
	 */
	private TripletsTable selectTriplet(Session session, long blkid) {
		Object[] pKey = new Object[2];
		pKey[0] = blkid;
		pKey[1] = null;
		return session.find(TripletsTable.class, pKey);
	...
	/** Delete a triplet using blockId
	 * @param session
	 * @param blkid
	 * @param index
	 */
	private void deleteTriplet(Session session, long blkid) {
		Object[] pKey = new Object[2];
		pKey[0] = blkid;
		pKey[1] = null; //TODO: see if this works
		session.deletePersistent(TripletsTable.class, pKey);
	}
	 ...
	 */
	private List<TripletsTable> selectTriplets(Session session, String datanodeName) {
		QueryBuilder qb = session.getQueryBuilder();
		QueryDomainType<TripletsTable> dobj = qb.createQueryDefinition(TripletsTable.class);
