...
//  NameNodeResourceChecker nnResourceChecker;
  ...
  // TODO JIM - replace this with a value stored in the database
  private volatile SafeModeInfo safeMode;  // safe mode information
  private long maxFsObjects = 0;          // maximum number of fs objects
  /**
   * The global generation stamp for this file system. [J] Now persisted to DB
   * and it will take care of the Global Timestamp
   */
...
//      cache.putRoot(this.dir.rootDir); //added for magic cache
  ...
  public boolean isLeader() {
    return nameNode == null ? false : nameNode.isLeader(); //FIXME: find out a better way of handling namenode format
  }
...
//  }
  ...
  private static Collection<URI> getStorageDirs(Configuration conf,
    ...
    if (startOpt == StartupOption.IMPORT) {
      ...
      if (dirNames.isEmpty()) {
        LOG.warn("!!! WARNING !!!"
                + "\n\tThe NameNode is currently running without persistent storage."
...
//      if (nnrmthread != null) nnrmthread.interrupt();
    ...
    if (DFSConfigKeys.DFS_INODE_CACHE_ENABLED) {
      MemcacheForINode.getInstance().close();
    }
  }
   ...
   * if necessary.
   */
  // TODO - Jude has an optimization where the INode is passaed as
  // a parameter and the lastBlock is taken from the Inode. 
  private LocatedBlocks getBlockLocationsUpdateTimes(String src,
    ...
    //try {
    long now = now();
    // TODO - Inefficient way to get the INode, see Jude's optimization. JIM
    ...
    if (doAccessTime && isAccessTimeSupported()) {
      // TODO - Ask Kamal/Hooman about this
...
//                INode[] inodesOnPath = null;
//
//                if (flag.contains(CreateFlag.APPEND)) {
//                    // use cache for append because the file should exist
//                    inodesOnPath = dir.getExistingPathINodes(src);
//                } else {
//                    // since we're creating the file, the file won't exist before 
//                    // in normal circumstances, therefore its useless to access the cache
//                    // instead, we cache only up to the parent directory level
//                    String pathToParent = new Path(src).getParent().toString();
//                    INode[] inodesOnPathToParent = dir.getExistingPathINodes(pathToParent);
//                    inodesOnPath = new INode[inodesOnPathToParent.length + 1];
//                    System.arraycopy(inodesOnPathToParent, 0, inodesOnPath, 0, inodesOnPathToParent.length);
//
//                    // if parent is not null
//                    if (inodesOnPath[inodesOnPath.length - 2] != null) {
//                        // check if the child exists already (for validation)
//                        inodesOnPath[inodesOnPath.length - 1] = INodeHelper.getINode(src.substring(src.lastIndexOf(Path.SEPARATOR_CHAR) + 1, src.length()), inodesOnPath[inodesOnPath.length - 2].getID());
//                    }
//                }
...
//          
//          // Recovery in progress exception will reassign the lease to the new client writing to it. This happens when the original client's lease hard limit expires
//          // Hence write operations occur even though we have exceptions and we need to commit them to db
//          if(ioe instanceof RecoveryInProgressException || ioe instanceof AlreadyBeingCreatedException || ioe instanceof AccessControlException) {
//            DBConnector.commit();
...
//          }
//          
//          // [J] This operation could fail in case of concurrent writes where there is a race condition by two clients to create a file or directory. So we retry again for a minimum amount of times
...
//        {
//          caughtExceptionAfterRetry = ex;
//          LOG.error("startFile("+src+"). Exception: "+ex.getMessage(), ex);
//          DBConnector.safeRollback();
//          DBConnector.setDefaultLock();
//        }
//      } // while loop
...
//        DBConnector.safeRollback();
//        DBConnector.setDefaultLock();
//        if(caughtExceptionAfterRetry instanceof IOException) {
//          throw (IOException)caughtExceptionAfterRetry;
//        }
//        else {
//          // Runtime exception
//          throw (ClusterJException)caughtExceptionAfterRetry;
//        }
//      }
//    } // end finally
// TODO END JIM
  ...
  private LocatedBlock startFileInternal(String src,
    ...
    if (!createParent) {
      verifyParentDir(src);
      // START JIM
//            verifyParentDir(src, inodesOnPath);
      // END JIM
    }
  ...
  LocatedBlock getAdditionalDatanode(final String src, final ExtendedBlock blk,
    ...
    TransactionalRequestHandler getAdditionalDatanodeHandler = new TransactionalRequestHandler(OperationType.GET_ADDITIONAL_DATANODE) {
      ...
      public Object performTask() throws PersistanceException, IOException {
        ...
        // TODO - Jim. Jude's code: clientnode = file.getClientNode();
...
//    }
   ...
   * deleted a small number at a time holding the {@link FSNamesystem} lock.
...
//                }
  ...
  DirectoryListing getListing(final String src, final byte[] startAfter,
    ...
    // TODO Jim - I would prefer to have templated the return type 
    // of TransactionalRequestHandler.performTask so that we don't
    // have this cast here.
...
//  }    
  ...
  public long getMissingBlocksCount() {
    ...
    try {
      ...
      // TODO: JIM - why not let the exception propagate?
      LOG.error(ex.getMessage(), ex);
      return -1;
    }
  }
...
//      return this.reached >= 0;
    ...
    private synchronized void leave(boolean checkForUpgrades) throws IOException, PersistanceException {
      ...
      // TODO JIM - use 'cluster' table to change this node's safe_mode state to false.
      if (checkForUpgrades) {
...
//              + blockManager.numOfUnderReplicatedBlocks() + " blocks");
    ...
    private synchronized void incrementSafeBlockCount(short replication) throws IOException, PersistanceException {
       ...
       * 'replicas' table. Hence, the condition 'replication == safeReplication'
       * can never be exactly equal, since after restart, we can have many
       * replicas (depending on the replication factor) This is just fetched from the
...
// @ClientProtocol
  ...
  void setGenerationStamp(long stamp) throws PersistanceException {
    EntityManager.update(new GenerationStamp(stamp));
  }
  ...
  long getGenerationStamp() throws PersistanceException {
    return EntityManager.find(GenerationStamp.Finder.Counter).getCounter();
  }
  ...
  private long nextGenerationStamp() throws SafeModeException, PersistanceException {
    ...
    long gs = getGenerationStamp();
    setGenerationStamp(++gs);
    return gs;
  }
