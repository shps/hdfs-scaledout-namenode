...
public class INodeDirectory extends INode {
    ...
    public INodeDirectory(INodeDirectory other) throws PersistanceException {
        ...
        // TODO - Does lazy fetch of children work?
...
//        return node;
    ...
    INode getNode(String path, boolean resolveLink)
            throws UnresolvedLinkException, PersistanceException {
        if (DFSConfigKeys.DFS_INODE_CACHE_ENABLED) {
            // Use Memcached to check if INode is in the path
            // TODO - mismatch with HK's code which pull up all resources at the start
            // of a transaction. We are now in the middle of the transaction and
            // we need resolve INodes on-demand.
            INode nodeFromCache =
                    MemcacheForINode.getInstance().resolve(path).getINode();
    ...
    public int getExistingPathINodes(byte[][] components, INode[] existing,
        ...
        INode curNode = this;
        StringBuilder memcachedPath = new StringBuilder(); // key used for caching
        ListOrderedMap<Long, INodeMemcachedEntry> inodes = new ListOrderedMap<Long, INodeMemcachedEntry>();
        // save the root INode
//        inodes.put(this.id, new INodeCachedEntry(this.id, this.getParentIDLocal(), this.getLocalName()));
        inodes.put(this.id, new INodeMemcachedEntry(this.id, this.getParentIDLocal(), this.getName()));
        // [J] by commenting this out, one of the test case fails. 
        // The problem is that it needs the updated value of 'root' INode (i.e. the nscount and dscount value) 
        // but its minor and inexpensive
        //curNode = INodeHelper.getINode(this.id);         
        ...
        while (count < components.length && curNode != null) {
            ...
            // if the current node is a symbolic link and if it is the last component in the path, 
            // we throw an UnresolvedPathException if 'resolveLink = true' (as per function definition)
            // if an intermediate component in the path is actually a symbolic link, then also we throw UnresolvedPathException. 
            // We cannot have a symbolic link in the intermediary components of the path
            if (curNode.isLink() && (!lastComp || (lastComp && resolveLink))) {
                ...
                if (NameNode.stateChangeLog.isDebugEnabled()) {
                    NameNode.stateChangeLog.debug("UnresolvedPathException "
                            ...
                            + " count: " + count + " symbolic link: " + link + " target: " + target
                            + " remainder: " + remainder);
                }
            ...
            if (curNode != null) {
                inodes.put(curNode.id, new INodeMemcachedEntry(curNode.id, curNode.parentid, curNode.getName()));
                memcachedPath.append(Path.SEPARATOR).append(curNode.getName());
            } else {
        ...
        if (DFSConfigKeys.DFS_INODE_CACHE_ENABLED) {
            // TODO - should we cache the parent's path as well? If we read/writes lots of files in a 
            // directory we won't hit the cache for the parent dir. Resolving to the parent using the cache
            // would be good enough, then doing a single index scan to find the file in NDB.
            if (memcachedPath.length() > 0 && inodes.size() > 0) {
                MemcacheForINode.getInstance().putINode(memcachedPath.toString(), inodes);
            }
        }
     ...
     * @param setModTime set modification time for the parent node not needed
     * when replaying the addition and the parent already has the proper mod
     * time
...
//            }
    ...
    public List<INode> getChildren() throws PersistanceException {
        if (children == null) {
            // TODO - JIM The cached children are never verified!??!
//            if (DFSConfigKeys.DFS_INODE_CACHE_ENABLED) {
//            children = MemcacheForINode.getInstance().resolveChildren(id).getChildren();
//            }
//            if (children != null) {
//                return children;
//            }
            children = (List<INode>) EntityManager.findList(INode.Finder.ByParentId, getId());
        }
        return children;
    }
