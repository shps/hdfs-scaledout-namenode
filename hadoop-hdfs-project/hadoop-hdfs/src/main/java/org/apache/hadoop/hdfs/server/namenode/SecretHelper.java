...
public class SecretHelper {
  ...
  /** Updates a blockKey in the database (handles the serialization of blockKey)
   * @param keyId
   * @param blockKey
   */
  public static void update(int keyId, BlockKey blockKey, short KEY_TYPE) {
    ...
    // updates the key's type and expiry date
    updateInternal(session, keyId, blockKey.getExpiryDate(), keyBytes.getData(), KEY_TYPE);
    session.flush();
  }
  ...
  public static void put(int keyId, BlockKey blockKey, short KEY_TYPE) {
	 ...
	 * @param blockKey
	 */
	public static void putWithTransaction(int keyId, BlockKey blockKey, short KEY_TYPE) {
	 ...
	 * @param keyId
	 */
	public static void removeKeyWithTransaction(int keyId) {
		int tries = RETRY_COUNT;
		boolean done = false;
		Session session = DBConnector.obtainSession();
		Transaction tx = session.currentTransaction();
  ...
  /** Updates the row in DelegationKey table.
   * @param session
   * @param keyId
   * @param expiryDate
   * @param keyBytes
   */
  private static void updateInternal(Session session, int keyId, long expiryDate, byte[] keyBytes, short keyType) {
    // find the existing row  
    DelegationKeyTable dkte = select(session, keyId);
      if (dkte != null) {
        dkte.setKeyType(keyType);
        dkte.setExpiryDate(expiryDate);
        dkte.setKeyBytes(keyBytes);
        update(session, dkte);
      }
      else {
